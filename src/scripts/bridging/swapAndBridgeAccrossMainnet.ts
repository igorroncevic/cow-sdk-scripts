import { base, arbitrum, APP_CODE, mainnet } from "../../const";
...
export async function run() {
  const sourceChain = SupportedChainId.MAINNET;
  const sellToken = mainnet.DAI_ADDRESS;
  const sellTokenDecimals = 18;
  const sellAmount = ethers.utils
    .parseUnits("50", sellTokenDecimals)
    .toString();
  const targetChain = SupportedChainId.ARBITRUM_ONE;

  const buyToken = arbitrum.USDC_ADDRESS;
